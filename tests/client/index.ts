...
async function callPromise<T = any>(methodName, ...args) {
  try {
...
describe('Client side testing', function () {
   ...
   */
  async function insertWithDifferentClients(count: number): Promise<
    {
      clientId: number;
  ...
  > {
    ...
    _.times(count, function (index) {
      ...
      promises.push(
        callWithConnectionPromise(connection, 'insertInvoice', ...params).then(
          (result) => {
            return {
              result,
              clientId,
  ...
  it('inserts correctly', async function () {
    ...
    for (var i = 0; i < USER_COUNT; ++i) {
      ...
      if (isFailingClient(clientId)) {
        ...
        const {
          result: { id, itemIds },
        } = results.find((r) => r.clientId === clientId) as {
  ...
  it('works concurrently', async function () {
    const invoiceId = await callPromise('insertInvoiceNoTransaction', {
      total: 50,
    });
